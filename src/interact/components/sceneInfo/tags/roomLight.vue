...
<template>
	<div class="room-lamp" v-show="toshow&&scope=='floor'&&store.building.currentFloor==4" :style="{left:pos.x+'px',top:pos.y+'px'}" @mousedown.stop="">
		<div class="info" :class="state" @mouseenter="show" @mouseleave="hide" v-el:info>
			...
			<div class="controls">
				<div class="control">
					<span>亮度</span>
					<ul class="adjust">
						<li class="l" @click="(intensity<=160)||intensity--">-</li>
						<li class="c" v-text="intensity"></li>
						<li class="r" @click="(intensity>=180)||intensity++">+</li>
					</ul>
				</div>
			</div>
			<div class="btn" :class="'btn'+state" @click="toggle">
				<div class="in">
					<span v-show="state=='off'">开灯</span>
					<span v-show="state=='on'">关灯</span>
					<span v-show="state=='error'">灯泡故障</span>
				</div>
			</div>
			<div class="d">
	...
	module.exports =
	{
		...
		methods:
		{
			...
			show()
			{
				if(this.state!='on') return;
				var ele = this.$els.info;
				$(ele).find('.controls').stop().animate({height:'36px'});
			},
			toggle()
			{
